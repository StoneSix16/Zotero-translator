...
async function _google(req: TranslateRequest) {
  ...
  let param = `sl=en&tl=zh-CN`;

  const xhr = await Zotero.HTTP.request(
    "GET",
    `https://translate.google.com/translate_a/single?client=gtx&${param}&hl=zh-CN&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&source=bh&ssel=0&tsel=0&kc=1&tk=${TL(
      req.value
    )}&q=${encodeURIComponent(req.value)}`,
    { responseType: "json" }
  );
  if (xhr?.status !== 200) {
